"""
Train Custom 11 Signs - HOLD SPACE TO RECORD
Signs: hello, welcome, please, run, sorry, thankyou, wait, sick, drink, happy, thirsty
"""
...
# Your 11 custom signs
SIGNS = ['hello', 'welcome', 'please', 'run', 'sorry', 'thankyou', 'wait', 'sick', 'drink', 'happy', 'thirsty']

# Hints for each gesture
HINTS = {
...
def collect_data():
    ...
    print("\n" + "="*60)
    print("CUSTOM 11 SIGNS TRAINING")
    print("="*60)
    print(f"Signs: {SIGNS}")
    ...
    for sign_idx, sign in enumerate(SIGNS):
        ...
        while samples_collected < SAMPLES_PER_SIGN:
            ...
            bar_width = 350
            cv2.rectangle(frame, (150, 30), (150 + bar_width, 50), (50, 50, 50), -1)
            cv2.rectangle(frame, (150, 30), (150 + int(bar_width * progress), 50), (0, 255, 0), -1)
            
            # Text
            cv2.putText(frame, f"{sign.upper()}", (10, 45), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 0), 3)
            cv2.putText(frame, f"[{sign_idx+1}/{len(SIGNS)}]", (520, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)
            cv2.putText(frame, f"{samples_collected}/{SAMPLES_PER_SIGN}", (520, 50),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            # Hint
            hint = HINTS.get(sign, '')
            cv2.putText(frame, hint, (10, 80), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 255, 100), 1)
            
            # Recording status
            if recording:
                cv2.putText(frame, "RECORDING...", (220, 465),
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv2.circle(frame, (200, 458), 10, (0, 0, 255), -1)
            else:
                cv2.putText(frame, "HOLD SPACE TO RECORD", (150, 465),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            
            # Hand status
            if results.multi_hand_landmarks:
                cv2.circle(frame, (610, 30), 12, (0, 255, 0), -1)
            else:
                cv2.circle(frame, (610, 30), 12, (0, 0, 255), -1)
            
            cv2.imshow('Training - 11 Signs', frame)
...
def main():
    print("\n" + "="*60)
    print("CUSTOM 11 SIGNS TRAINER")
    print("="*60)
    print(f"\nSigns to train: {SIGNS}")
    print(f"Total: {len(SIGNS)} signs")
